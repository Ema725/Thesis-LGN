...
int main(int argcc, char **argvv, char **envp) {
	...
	} else if (problem_type == HOLLAND_ROYAL_ROAD) {  
        if constexpr (Validation::validate_hrr_type()) {
            initializer = std::make_shared<
                    HollandRoyalRoadInitializer<EVALUATION_TYPE, GENOME_TYPE,
                            FITNESS_TYPE>>(data_file);
        } else {
            throw std::invalid_argument(
	...
	try { 
        if (checkpoint_file == "") {
            ...
            evolver->run();
            std::cout << "[DEBUG] evolver->run() is terminated." << std::endl; // <<< DEBUG PRINT
        } else {
            std::cout << "[DEBUG] i'm calling evolver->resume()..." << std::endl; // <<< DEBUG PRINT
            evolver->resume(checkpoint_file);
            std::cout << "[DEBUG] evolver->resume() is terminated." << std::endl; // <<< DEBUG PRINT
        }
    } 
    catch (const std::exception &e) { 
        std::cerr << "\n!!!! CRITICAL ERROR (std::exception) !!!!\n" << std::endl;
    ...
    std::cout << "[DEBUG] End of main program." << std::endl;
}
