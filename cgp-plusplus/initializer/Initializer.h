...
void Initializer<E, G, F>::init_parfile_parameters(std::string parfile_path) {
	...
	std::string extension = std::filesystem::path(parfile_path).extension().string();  //aggiunta del .string()
	...
	if (ifs.is_open()) {
		...
		while (ifs >> parameter >> value) {
			...
			} else if (parameter == "levels_back") {
				this->parameters->set_levels_back(value);
...
template<class E, class G, class F>
const std::shared_ptr<Evaluator<E, G, F>>& Initializer<E, G, F>::get_evaluator() const {
    return evaluator;
}
